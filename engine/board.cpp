...
bool Board::can_move_to_cell_in_the_middle(int desired_cell_index, CELL_DIRECTION current_car_direction)
{
...
void Board::update_positions()
{
  ...
  for (int i = 0; i < grid.size(); ++i)
  {
    ...
    case CELL_DIRECTION::DOWN:
      ...
      if (is_cell_in_the_middle_of_cross(new_cell_position_index) && new_calculated_speed > MAX_SPEED_IN_THE_CROSS)
      {
        new_calculated_speed = MAX_SPEED_IN_THE_CROSS;
        new_cell_position_index = i + new_calculated_speed * BOARD_SIZE;
      }
    ...
    if (is_cell_in_the_middle_of_cross(new_cell_position_index))
    {
      ...
      //sprawdz czy skrecanie jest wymagane
      if (grid[i]->is_turning_required())
      {
        // tutaj odbywa sie skrecanie
        if (new_is_staying)
        {
          // 1. sprawdz czy moze wjechac na skrzyzowanie
          ...
          if (grid[i]->get_current_car_direction() == CELL_DIRECTION::DOWN && grid[i]->get_desired_car_direction() == CELL_DIRECTION::LEFT)
          {
            int desired_cell_index = i + BOARD_SIZE;
            if (can_move_to_cell_in_the_middle(desired_cell_index, CELL_DIRECTION::DOWN))
            {
          ...
          else if (grid[i]->get_current_car_direction() == CELL_DIRECTION::UP && grid[i]->get_desired_car_direction() == CELL_DIRECTION::RIGHT)
          {
            int desired_cell_index = i - BOARD_SIZE;
            if (can_move_to_cell_in_the_middle(desired_cell_index, CELL_DIRECTION::UP))
            {
              new_cell_position_index = desired_cell_index;
              has_turned_in_this_round = true;
              new_is_staying = false;
          ...
          else if (grid[i]->get_current_car_direction() == CELL_DIRECTION::LEFT && grid[i]->get_desired_car_direction() == CELL_DIRECTION::UP)
          {
            int desired_cell_index = i - 1;
            if (can_move_to_cell_in_the_middle(desired_cell_index, CELL_DIRECTION::LEFT))
            {
              new_cell_position_index = desired_cell_index;
              has_turned_in_this_round = true;
              new_is_staying = false;
          ...
          else if (grid[i]->get_current_car_direction() == CELL_DIRECTION::RIGHT && grid[i]->get_desired_car_direction() == CELL_DIRECTION::DOWN)
          {
            int desired_cell_index = i + 1;
            if (can_move_to_cell_in_the_middle(desired_cell_index, CELL_DIRECTION::RIGHT))
            {
              new_cell_position_index = desired_cell_index;
              has_turned_in_this_round = true;
              new_is_staying = false;
        ...
        // 3. jezeli tak oraz auto nie zatrzymalo sie tutaj w poprzedniej kolejce, to zatrzymaj się na pierwszym wolnym miejscu przed skrzyzowaniem
        else
        {
          int i_temp = i;
          ...
          while (true)
          {
            ...
            if (grid[i]->get_current_car_direction() == CELL_DIRECTION::LEFT && grid[i]->get_desired_car_direction() == CELL_DIRECTION::UP)
              i_temp--;
            if (grid[i]->get_current_car_direction() == CELL_DIRECTION::RIGHT && grid[i]->get_desired_car_direction() == CELL_DIRECTION::DOWN)
              i_temp++;
            if (grid[i]->get_current_car_direction() == CELL_DIRECTION::DOWN && grid[i]->get_desired_car_direction() == CELL_DIRECTION::LEFT)
              i_temp += BOARD_SIZE;
            if (grid[i]->get_current_car_direction() == CELL_DIRECTION::UP && grid[i]->get_desired_car_direction() == CELL_DIRECTION::RIGHT)
              i_temp -= BOARD_SIZE;
          }
        }
      }
